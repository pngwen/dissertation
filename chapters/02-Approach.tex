...
$\tens{D}_{ijk}$ counts the number of occurences of the phrase
...
Algorith~\ref{alg:BuildTensor} is a set of sparse tensors representing
...
these alternating negative and postitive factors would necessarily
...
decomposition. By constraining the CCD modle to use a square identity
...
factor with the mininmum distance.  The only issue with this approach
...
this reason, two steps are needed.  First, the mininum is found,
...
other hand, the minimum distance is above the threhshold it is not
assigned a source.
...
order by frequency, the vocabulary can be shortened with mininal
...
entries.  Even in dense format, this can be comfortably accomodated by
...
The sptensor libary is implemented as a shared library, and also has a
...
\item[{\tt sptensor}] A sparse tensor storage structure.  Tensors are stored
  as a list of coordinates with indexes and values stored in separte
...
\item[{\tt tensor\_view}] A series of overlays for tensor objects.  These
  ...
  Mechanisms are provided to allow the libray's user to provide their
  own tensor views.
...
tensors are fairly ineffecient both in time and space complexity.
...
\subsection{Text Modelling Suite}
